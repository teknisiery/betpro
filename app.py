...
def extract_features_from_files(temp_dir):
    ...
    def get_result(home_goals, away_goals, perspective='home'):
        if perspective == 'home':
            if home_goals > away_goals: return 'W'
            elif home_goals == away_goals: return 'D'
            else: return 'L'
        else:
            if away_goals > home_goals: return 'W'
            elif away_goals == home_goals: return 'D'
            else: return 'L'
...
async def predict(zip_file: UploadFile = File(...)):
    ...
    # Ekstrak ZIP ke folder yang sama (temp_dir)
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        zip_ref.extractall(temp_dir)

    # Hapus file ZIP-nya (opsional)
    os.remove(zip_path)

    # Proses fitur dari file yang sudah diekstrak
    ...
    # Ambil display handicap dan OU dari 01_info.csv
    info = pd.read_csv(os.path.join(temp_dir, "01_info.csv"))
    ah_str = info[info['0'] == 'Pre-game AH']['1'].values[0]
    ou_str = info[info['0'] == 'Pre-game O/U']['1'].values[0]
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port)    parts_ou = ou_str.split('/')
    over_odds = float(parts_ou[0].strip().split()[-1])
    ou_line = float(parts_ou[1].strip())
    under_odds = float(parts_ou[2].strip().split()[-1])
    ...
    for name, df in [('home', form_home), ('away', form_away)]:
        ...
        if name == 'home':
            ...
            features['home_form_pts'] = last5['Result_Sassuolo'].map({'W': 3, 'D': 1, 'L': 0}).sum()
        else:
            features['away_avg_goals_scored'] = last5['AG'].mean()
            features['away_avg_goals_conceded'] = last5['HG'].mean()
            features['away_form_pts'] = last5['Result_Milan'].map({'W': 3, 'D': 1, 'L': 0}).sum()
...
async def predict(files: list[UploadFile] = File(...)):
    ...
    return JSONResponse({
        ...
        "btts": btts,
        "over_ht": over_ht,
        "features": features   # <-- sudah termasuk
    })
