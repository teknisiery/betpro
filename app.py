...
def compute_ou_actual(ft_home, ft_away, ou_line):
    ...
    # Garis penuh (2.0, 2.5, 3.0) -> tidak ada menang/kalah setengah
    is_full_line = (ou_line * 100) % 50 == 0
    if is_full_line:
        if total > ou_line:
            return ('over', 1.0)
        elif total < ou_line:
            return ('under', 1.0)
        else:  # hanya mungkin untuk garis bulat (2.0, 3.0)
            return ('push', 0.0)
    else:
        # Garis setengah (2.25, 2.75)
        if (ou_line * 100) % 100 == 25:   # contoh 2.25
            lower = int(ou_line)
    ...
    try:
        ...
        for key in required_odds:
            if key not in features or features[key] is None:
                return JSONResponse({"error": f"Fitur odds '{key}' tidak ditemukan. Upload ulang ZIP dan tekan 'Dapatkan Rekomendasi' lagi."}, status_code=400)
            if features[key] == 1.0:
