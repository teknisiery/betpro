...
def parse_ah_line(ah_str):
    # PERBAIKAN: gunakan pemisah " / " agar handicap dengan "/" tidak terpecah
    segments = ah_str.strip().split(" / ")
    if len(segments) < 3:
        raise ValueError(f"Format AH tidak valid: {ah_str}")
    home_odds = float(segments[0].strip().split()[-1])
    handicap_text = segments[1].strip()
    away_odds = float(segments[2].strip().split()[-1])
    return home_odds, handicap_text, away_odds

def parse_ou_line(ou_str):
    # PERBAIKAN: gunakan pemisah " / "
    segments = ou_str.strip().split(" / ")
    if len(segments) < 3:
        raise ValueError(f"Format O/U tidak valid: {ou_str}")
    over_odds = float(segments[0].strip().split()[-1])
    line = float(segments[1].strip())
    under_odds = float(segments[2].strip().split()[-1])
    return over_odds, line, under_odds

def parse_btts_line(btts_str):
    parts = btts_str.strip().split('/')
...
def compute_ah_actual(ft_home, ft_away, handicap):
    ...
    if diff > 0:
        return ('home', 1.0) if diff >= 0.5 else ('home', 0.5)
    elif diff < 0:
        return ('away', 1.0) if diff <= -0.5 else ('away', 0.5)
    else:
        return ('push', 0.0)
...
def compute_ou_actual(ft_home, ft_away, ou_line):
    ...
    if ou_line % 1 == 0:
        if total > ou_line:      return ('over', 1.0)
        elif total < ou_line:    return ('under', 1.0)
        else:                    return ('push', 0.0)
    else:
        if (ou_line - int(ou_line)) == 0.25:
            lower = int(ou_line)
            upper = int(ou_line) + 0.5
        else:
            lower = int(ou_line) + 0.5
            upper = int(ou_line) + 1.0
        if total > upper:        return ('over', 1.0)
        elif total < lower:      return ('under', 1.0)
        elif total == upper:     return ('over', 0.5)
        elif total == lower:     return ('under', 0.5)
        else:                    return ('push', 0.0)
...
def extract_features_from_files(temp_dir):
    ...
    if mask_live_btts.any():
        btts_yes, btts_no = parse_btts_line(info.loc[mask_live_btts, 1].values[0])
    if mask_live_over_ht.any():
        ht_yes, ht_no = parse_btts_line(info.loc[mask_live_over_ht, 1].values[0])
...
async def feedback(
    ...
    try:
        ...
        # Validasi odds wajib ada
        required_odds = ['ah_home_odds', 'ah_away_odds', 'over_odds', 'under_odds']
        for key in required_odds:
            if key not in features or features[key] is None:
                return JSONResponse({"error": f"Fitur odds '{key}' tidak ditemukan. Pastikan Anda mengupload ulang ZIP dan menekan 'Dapatkan Rekomendasi' lagi."}, status_code=400)
            if features[key] == 1.0:
                return JSONResponse({"error": f"Odds '{key}' bernilai 1.0 (tidak valid). Cek file CSV atau ulangi proses."}, status_code=400)
        ...
        # --- PROFIT AH (dengan odds nyata) ---
        ah_home_odds = float(features['ah_home_odds'])
        ah_away_odds = float(features['ah_away_odds'])
        if actual_ah != 'push':
            if pred_ah == actual_ah:
                if actual_ah == 'home':
                    profit_ah = (ah_home_odds - 1) * 100 * ah_factor
                else:
                    profit_ah = (ah_away_odds - 1) * 100 * ah_factor
            else:
                profit_ah = -100 * ah_factor
        else:
            profit_ah = 0.0

        # --- PROFIT OU (dengan odds nyata) ---
        over_odds = float(features['over_odds'])
        under_odds = float(features['under_odds'])
        if actual_ou != 'push':
