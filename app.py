...
def compute_ah_actual(ft_home, ft_away, handicap):
    ...
    Mengembalikan (pemenang, faktor_profit).
    """
...
def compute_ou_actual(ft_home, ft_away, ou_line):
    """
    Menghitung hasil aktual Over/Under.
    Mengembalikan (hasil, faktor_profit).
    """
    ...
    try:
        ...
        if actual_ou != 'push':
            if pred_ou == actual_ou:
                ...
                # Untuk garis bulat, kalah penuh; untuk garis pecahan, kalah setengah/penuh sesuai ou_factor
                if ou_line % 1 == 0.0:
                    profit_ou = -100
                else:
